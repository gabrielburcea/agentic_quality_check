...
DATABRICKS_MODELS = {
    ...
    # LLM for Agents via Databricls Model Serving
    'llm': [
        'provider': 'databricks', 
    ...
    # Alternative: Custome fine-tuned model (if I train one later?)
    'llm_custom': [
        'provider': 'databricks', 
...
FREE_MODELS = {
    ...
    # LLM from HugginFace (smaller models fit in memory)
    'llm': {
        'provider': 'huggingface', 
        'model_name': 'microsoft/phi-2',    # Phi-2: 2.7B params, good for reasoning
        'temperature': 0.0, 
        'max_tokens': 2048, 
        'use_case': 'Local infernce for development/testing'
    },
    # Alternative smalle LLM
    "llm_tiny":{
        'provider': 'huggingface', 
...
RAG_CONFIG = {
    ...
    'fixed_size': {
        ...
        'chunck_overlap': 100,    # Overlap for context preservation
        'use_case': 'Simple, fast but may break semantic units'
    }, 

    # Vector store configuration 
    'vector_store': {
        # Databricls Vector earch (manages service)
        'databricks': {
            'provider': 'databricks_vector_search', 
            'endpoint_name': 'agentic_quality_check_vs_endopoint', 
        ...
        'active': 'databricks'.  # Chance to 'faisss' for free option
    }, 

    # Retrieval parameters
    'retrieval': {
        'top_k': 5    # Number of chunks to retrieve per query
        'rerank': True,    # Re-rank retrieve chunks using LLM
...
MAPPING_CONFIG = {
    ...
    'storage': {
        'format': 'json'   # Json or Delat table
        'path': f"{PATHS['mappings']}headline_csv_mappings.json", 'delta_trable': f"{UC_CONFIG['catalog']}.{UC_CONFIG['schema']}.headline_mappings"
    }, 
    ...
    'schema': {
        ...
        'created_ by': 'str',   # User who created the mapping 
    ...
    'auto_suggest': {
        'enabled': True, 
        'method': 'semantic_similarity'm     # Match headline text to column names/descriptions
        'confidence_threshold': 0.8,     # Only suggest if similarity if 0.8
...
AGENT_CONFIG = {
    ...
    'style_agent': {
        'name': 'Style & Writting Quality Checker', 
        'model': ACTIVE_MODELS['llm'], 
        ...
        'rag_enabled': True,    # Retrive similar sections for consistency
        'temperature': 0.0    
        'max_retries': 2
    }, 
    ...
    'self_healing_agent': {
        ...
        'learning_rate': 0.1,    # How much to weight recent feedbakc
        'min_feedback_samples': 5   # Minimum feedbakc before updating prompts 
    }
}
...
STREAMLIT_CONFIG = {
    ...
    # Session state keys (for storing data across interations)
    'session_keys':{
...
def get_volume_path(volume_type: str) -> str:
    """
    Get the full Unity Catalog Volumne path for a given volume type. 

    Args:
        volume_type (str): One of 'pdfs', 'cvs', 'processed', 'mappings, 'feedback'
    Returns:
        str: Full path lile '/Volumes/my_catalog/agentic_quality_check_dev/csvs_volume/'
    Example:
        >>> get volume_path('pdfs') 'Volumes/main/agentic_quality_check_dev/pdf_volume/'
    """
...
def get_active_model(model_type: str) -> dict:
    ...
    Returns:
        dict: Model configurationdictionary
    Example:
        >>> get_active_model('embedding'){'provider': 'databricks', 'model_name': 'databricks-bge-large_en', ...}
    """
...
def switch_to_free_models():
    """
    Swithc all active models to free/open-source alternatives.
    Useful for developemt or when running locally
    """
...
def switch_to_databricks_models():
    """
    Switch all active models to Databricks mandage services 
