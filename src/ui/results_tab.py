"""
Results Tab - Table Extraction and Verification Results
...
Architecture Alignment:
    ...
    - Process: TableExtractorAgent extraction (Layer 4)
    - Output: Extracted tables + lineage JSON for verification agents
"""
...
def render_results_tab():
    ...
    Workflow:
        ...
        6. Save results as JSON for downstream agents
    """
    ...
    csvs_dir = "/tmp/"  # CSVs are uploaded to /tmp in your app
    ...
    if not mappings_path.exists() or not list(mappings_path.glob("*.json")):
        st.warning("⚠️ No mappings found. Please create mappings in Tab 3 first.")
        return

    # Step 1: List available mappings
    st.subheader("1️⃣ Select Mapping File")
    ...
    st.subheader("2️⃣ Extract Tables")

    # API Key input
    api_key = st.text_input(
        "Anthropic API Key",
        type="password",
        help="Enter your Claude API key (from console.anthropic.com)",
    ...
    if st.button("🚀 Run Extraction", type='primary', disabled=not api_key):
        if not api_key:
        ...
        with st.spinner("🤖 Extracting tables... This may take a minute"):
            results = []

            for idx, headline_mapping in enumerate(mapping_data['mappings']):
                st.write(f"Processing {idx + 1}/{len(mapping_data['mappings'])}: {headline_mapping['headline_text']}")

                # Get first paragraph and CSV
                paragraph = headline_mapping['paragraphs'][0] if headline_mapping['paragraphs'] else ""
                csv_file = headline_mapping['csv_files'][0] if headline_mapping['csv_files'] else ""
                ...
                # Load CSV
                csv_path = Path(csvs_dir) / csv_file
                ...
                try:
                    ...
                    csv_metadata = get_csv_metadata(str(csv_path))
                    column_metadata = csv_metadata['columns']
    ...
    if 'extraction_results' in st.session_state:
        st.subheader("3️⃣ Extracted Tables")
        ...
        for idx, result in enumerate(results, 1):
            with st.expander(f"📊 {idx}. {result.get('headline_text', 'Unknown')}", expanded=(idx == 1)):
                ...
                if result.get('extracted_table'):
                    df_result = pd.DataFrame(result['extracted_table'])
        ...
        # Step 4: Save results as JSON for downstream agents
        st.subheader("4️⃣ Save Results for Verification")
        st.write("Save extraction results as JSON for the Numerical and Style Agents")

        if st.button("💾 Save Extraction Results as JSON", type="primary"):
            # Build JSON structure for downstream agents
            extraction_output = {
